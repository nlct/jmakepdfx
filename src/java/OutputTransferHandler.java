...
public class OutputTransferHandler extends TransferHandler
{
   public OutputTransferHandler(Jmakepdfx jmakepdfx)
   {
      super();
      this.jmakepdfx = jmakepdfx;
   }
   ...
   public boolean importData(TransferHandler.TransferSupport supp)
   {
      ...
      try
      {
         ...
         for (File f : list)
         {
            if (jmakepdfx.supportsOutput(f))
            {
               jmakepdfx.setOutputFile(f);
               return true;
            }
         }
      }
